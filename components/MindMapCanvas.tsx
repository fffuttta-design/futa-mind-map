...
function makeEdgePath(x1: number, y1: number, x2: number, y2: number, v: boolean, style: "curve" | "straight" = "curve"): string {
  ...
  // 横向き：親から水平に出て、中間でストンと急降下し、子へ水平に入る参考マインドマップ型のS字。
  // 制御点の x を両端とも「中点」に置くことで、遠い（＝上下端の）子ほど中間の縦落ちが急になり、
  // 一番下の子は"ストン"と直角に近い軌道で子へ入る。以前は水平助走を40pxで頭打ちにしていたため、
  // 中間が長い直線の斜めになり「S字っぽくない」不具合になっていた。
  const cx = (x1 + x2) / 2;
