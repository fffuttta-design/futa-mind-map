...
- **ノートノード（noteContent）**: 本文を持つカード。キャンバス上では**プレビュー**（簡易 Markdown：見出し・箇条書き等をパース描画、続きはフェードアウト）。**四隅のハンドルで幅・高さを自由にリサイズ**（`customWidth`/`customHeight`）。**ダブルクリックで前面の大きなリッチエディタ（TipTap）が開き、リアルタイムプレビューで編集**（`# `→見出し、`- `→箇条書き、`**太字**` 等がその場で整形。ツールバー＝B/I/U/S・文字色・マーカー・見出し・リスト・Todo・引用・コード・区切り線。700ms デバウンス＋閉じる時に自動保存）。本文は **TipTap JSON** で保存（`components/NoteEditor.tsx`）。カードのプレビューやホバー全文表示は `lib/noteMarkdown.ts` の `noteContentToMarkdown`（JSON→簡易Markdown）で供給するため**カードの見た目は従来どおり**。旧・簡易Markdown文字列のノートも自動で読み込む（`markdownToTiptapHTML`）。エディタは**閲覧権限（readOnly）でも開けて全文を読める**（編集できるのは編集権限者のみ＝ツールバー非表示・editable=false）。ホバーの小プレビューも維持（クリックでエディタへ）。
