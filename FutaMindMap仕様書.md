...
>
> - 最終更新基準: アプリ v1.12.0 / デスクトップ v1.3.0（NSIS + electron-updater へ移行）
...
- **ノートノード（noteContent）**: 本文を持つカード。キャンバス上では**プレビュー**（簡易 Markdown：見出し h1/h2・箇条書き等をパース描画、続きはフェードアウト）。**四隅のハンドルで幅・高さを自由にリサイズ**（`customWidth`/`customHeight`）。**ダブルクリックで前面の編集モーダルを開いて本文を編集**（左＝Markdown 入力／右＝ライブプレビューの2ペイン・600ms デバウンス自動保存・Esc/×/背景クリックで閉じる）。モーダルは**閲覧権限（readOnly）でも開けて全文を読める**（編集できるのは編集権限者のみ＝プレビュー1ペイン表示）。ホバーで小さな全文プレビューも出る（クリックでモーダルへ）。※将来もっとリッチにしたい場合は FutaMemo 同様の TipTap エディタへ差し替え可能（現状は軽量 Markdown 版）。
