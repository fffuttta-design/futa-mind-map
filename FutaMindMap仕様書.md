...
>
> - 最終更新基準: アプリ v1.35.1（同じ親にぶら下がる複数の枝の起点を縦に少し散らして線の重なりを軽減）/ デスクトップ v1.3.0（NSIS + electron-updater へ移行）
...
- **接続線（エッジ）**: 親子を曲線（curve）または直線（straight）で結ぶ（ページ設定で切替）。枠線太さもページ設定で 0/1/2/3px。
  ...
  - **枝の起点を縦に散らして重なりを軽減**: 同じ親の同じ側に横向きの子が複数ぶら下がると、枝が全部 −玉の1点から出て途中で束になり重なる。そこで −玉に付く横向きの子を y で並べ、起点を玉のまわりに少し縦に散らす（間隔15px・全体48px上限で玉から離れて見えない範囲、子1つなら従来どおり中心）。`collapseHubOrigin` が算出。曲線が気になる場合はページ設定で直線（straight）に切り替えると、共通起点から各子へ真っ直ぐ伸びるため重なりは出ない。
