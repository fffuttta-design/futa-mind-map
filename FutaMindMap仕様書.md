...
>
> - 最終更新基準: アプリ v1.10.0 / デスクトップ v1.3.0（NSIS + electron-updater へ移行）
...
- **ノートノード（noteContent）**: 本文を持つカード。キャンバス上では**プレビュー**（簡易 Markdown：見出し h1/h2・箇条書き等をパース描画、続きはフェードアウト）。**四隅のハンドルで幅・高さを自由にリサイズ**（`customWidth`/`customHeight`）。**ダブルクリックでポップアップを開いてそこで本文を編集**（Markdown・自動保存）。ポップアップは**閲覧権限（readOnly）でも表示**され、編集モードに入るのは編集権限者のみ。ホバーでも全文プレビューが出る。
