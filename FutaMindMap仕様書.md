...
### 5-1. ノード（MindMapNode）
- **形状（shape）**: `pill`（カプセル）/ `rect`（四角）/ `circle`（円）/ `diamond`（ダイヤ）/ `text`（枠なし＝テキストのみ）。**`text`（枠なし）ノードは枝を「テキストの下線」として引く**（`branchPath`）＝箱を持たず線の上に文字が乗る手描き風（オーガニック）のマインドマップになる。枝の下線は画面表示・SVG/PNG書き出しの両方に反映。右クリック→🎨見た目→「形」の一番右「T」で切り替え（ツールチップ「枠なし（テキストのみ）」）。
