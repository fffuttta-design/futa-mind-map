...
>
> - 最終更新基準: アプリ v1.36.0（接続線の既定を「直線」に変更＝玉から各子へ真っ直ぐ・線の交差を解消）/ デスクトップ v1.3.0（NSIS + electron-updater へ移行）
...
- **接続線（エッジ）**: 親子を曲線（curve）または直線（straight）で結ぶ（ページ設定で切替）。枠線太さもページ設定で 0/1/2/3px。**既定は「直線」（v1.36.0〜）**＝玉から各子へ真っ直ぐ引く。1点（−玉）から出た直線どうしは交差しないため、同じ親に子が縦に複数並んでも線が重ならず、下の子ほど急角度になる（曲線S字は遠近で曲がるタイミングがズレて上下順が入れ替わり交差していた）。曲線に戻したいマップはページ設定で「曲線」に切替。
