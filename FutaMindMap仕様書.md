...
- **ヘッダー**: 戻る／タイトル編集／**主要アクションのみ常時表示（✨AIで生成・🔒共有）**＋**「☰ メニュー」ドロップダウン**に副次操作をまとめる（🗺️ページ設定・🏷️マスタ管理・🕐履歴・SVG/PNG書き出し）／右端に🔗IDコピー・⚙️アプリ設定・**保存ステータス表示**（自動保存・保存待ち・保存中・保存済み・保存失敗）。ボタンが増えて乱雑にならないよう整理。
