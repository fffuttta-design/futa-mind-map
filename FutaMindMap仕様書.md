...
>
> - 最終更新基準: アプリ v1.37.0（接続線の既定を「曲線」に戻し、曲線を参考マインドマップ型に改善＝親から水平に出て子へ水平に入り、上下端の遠い子ほど急角度で扇状に交差しにくい）/ デスクトップ v1.3.0（NSIS + electron-updater へ移行）
...
- **接続線（エッジ）**: 親子を曲線（curve）または直線（straight）で結ぶ（ページ設定で切替）。枠線太さもページ設定で 0/1/2/3px。**既定は「曲線」**。横向きの曲線は**親から水平に出て、子へ水平に入る**参考マインドマップ型のS字で描く（v1.37.0〜）。**出入りの水平助走を一定量（最大40px）に抑える**ことで、共通の玉（−玉）から複数の子へ扇状に線が出ても**交差しにくく**、上下端の遠い子ほど中間が急な斜めになる（＝「一番下の子が一番急角度で重ならない」参考画像の見た目）。カクカクさせたいマップはページ設定で「直線」に切替。
