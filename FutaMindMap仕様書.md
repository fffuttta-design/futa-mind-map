...
>
> - 最終更新基準: アプリ v1.37.1（横向き曲線を本来のS字に修正＝制御点xを中点に置き、親から水平に出て中間でストンと急降下し子へ水平に入る。v1.37.0の40px頭打ちで中間が直線斜めになりS字にならなかった不具合を修正）/ デスクトップ v1.3.0（NSIS + electron-updater へ移行）
...
| ノート編集 | TipTap 3（リッチエディタ・WYSIWYG）。本文は TipTap JSON で保存し、カード表示用に簡易Markdownへ変換 |
...
- **ヘッダー**: 戻る／タイトル編集／**すべての操作を「☰ メニュー」ドロップダウンに集約**（✨AIで生成・🔒共有・🗺️ページ設定・🏷️マスタ管理・🕐履歴・SVG/PNG書き出し）／右端に🔗IDコピー・⚙️アプリ設定。**保存ステータスの常時表示は廃止**（自動保存は継続。**保存失敗したときだけ**赤字で警告を出す＝保存漏れ検知用）。ボタンが増えて乱雑にならないよう最小化。
...
### 5-1. ノード（MindMapNode）
- **形状（shape）**: `pill`（カプセル）/ `rect`（四角）/ `circle`（円）/ `diamond`（ダイヤ）/ `text`（枠なし＝テキストのみ）。**`text`（枠なし）ノードは枝を「テキストの下線」として引く**（`branchPath`）＝箱を持たず線の上に文字が乗る手描き風（オーガニック）のマインドマップになる。枝の下線は画面表示・SVG/PNG書き出しの両方に反映。右クリック→🎨見た目→「形」の一番右「T」で切り替え（ツールチップ「枠なし（テキストのみ）」）。**ページ設定の「✍️ 手描き風（枠なし）」トグル（`organicStyle`）で全体に一括適用**でき、中心（親なし）以外の全ノードを枠なし＋枝下線で描く（画面・書き出し共通）。折りたたみ玉（−ボタン）がある側の枝は**玉の中心を起点**にして描き、線が玉を突き抜けて見えないようにする（`collapseHubOrigin`）。
- **書式**: 背景色、文字色、太字、斜体、フォントサイズ（9〜28）。**フォントサイズを変えると箱（幅・高さ・行間）も自動で追従**し、文字が見切れない（手動リサイズ `customWidth/customHeight` を設定済みのノードは手動サイズを優先）。
...
- **枝を左右反転**: 右クリックメニュー「⇄ 枝を左右反転（親の反対側へ）」で、そのノード＋子孫すべてを**親ノードのx座標を軸に鏡像化**し、枝まるごと右↔左を入れ替える（親を持つノードのみ表示）。エッジの左右はノードのx相対位置で自動的に付け替わるため、片側に寄せたい枝を1クリックで反対側へ移せる。Undo対応。
- **折りたたみ（collapsed）**: 子ノードの表示/非表示。
- **接続線（エッジ）**: 親子を曲線（curve）または直線（straight）で結ぶ（ページ設定で切替）。枠線太さもページ設定で 0/1/2/3px。**既定は「曲線」**。横向きの曲線は**親から水平に出て、中間でストンと急降下し、子へ水平に入る**参考マインドマップ型のS字（`C 中点x,y1 中点x,y2 x2,y2`）。制御点xを両端とも中点に置くため、**上下端の遠い子ほど中間の縦落ちが急**になり、一番下の子は"直角に近い"軌道で子へ入る。カクカクさせたいマップはページ設定で「直線」に切替。
  - **真上／真下の子は縦の直線で接続**: 子が親のほぼ真上・真下にある（縦エッジ）ときは、枝を折りたたみ玉（−）ではなく**親の上下中央**から生やす。横の玉を起点にすると玉→真上へS字にくねり「右所属」に見えてしまうため。ドラッグ時の 8px スナップで子の中心x を親に合わせれば、真っ直ぐな縦線になる。
  - **縦横の判定は45°境界**: 子が横（左右所属）か縦（上下所属）かは `|dx| >= |dy|` で横、それ以外で縦とする（親子の中心の差 dx/dy）。以前は `|dx|*2 >= |dy|` で横を優先し過ぎ、斜め下に置くとすぐ右所属になり縦線にできなかった。45°境界にして斜め下の子も縦線として付けやすくした。
- **色プリセット**: ノード色・右クリックメニューとも **21色**（虹順＋グレー階調＋白／黒）。ツールバーは7×3、右クリックメニューも7列。
...
- **ノートノード（noteContent）**: 本文を持つカード。キャンバス上では**プレビュー**（簡易 Markdown：見出し・箇条書き等をパース描画、続きはフェードアウト）。**四隅のハンドルで幅・高さを自由にリサイズ**（`customWidth`/`customHeight`）。**ダブルクリックで前面の大きなリッチエディタ（TipTap）が開き、リアルタイムプレビューで編集**（`# `→見出し、`- `→箇条書き、`**太字**` 等がその場で整形。ツールバー＝B/I/U/S・文字色・マーカー・見出し・リスト・Todo・引用・コード・区切り線。700ms デバウンス＋閉じる時に自動保存）。本文は **TipTap JSON** で保存（`components/NoteEditor.tsx`）。カードのプレビューやホバー全文表示は `lib/noteMarkdown.ts` の `noteContentToMarkdown`（JSON→簡易Markdown）で供給するため**カードの見た目は従来どおり**。旧・簡易Markdown文字列のノートも自動で読み込む（`markdownToTiptapHTML`）。エディタは**閲覧権限（readOnly）でも開けて全文を読める**（編集できるのは編集権限者のみ＝ツールバー非表示・editable=false）。ホバーの小プレビューも維持（クリックでエディタへ）。**ヘッダーのタイトルはノード枠を自動で広げて表示**し、名前が見切れない（`noteTitleDisplay`／`nodeWidth`）。**前面エディタのヘッダーに「🗑ノートを削除」ボタン**があり、押すと確認のうえ**通常ノードに戻る**（本文は破棄／Undoで復元可）。同じ操作はノードの右クリックメニューからも可能。
...
- **線ラベル（label）**: 関連線を**ダブルクリック**すると中央にラベル文字を入力できる（親子エッジと同じ仕様。編集ボックスの −／＋ で大きさ変更＝`labelFontSize`）。ラベルがある時は ✕ ボタンはラベルの右隣にずれる。空文字で確定すると削除。
- **データ**: `MindMap.connections: Connection[]`（`{ id, from, to, color?, label?, labelFontSize? }`）。ノード削除時は両端を参照する線も自動で掃除。Undo/Redo・SVG/PNG エクスポート・公開ページにも反映。
...
### 5-5. 操作・インタラクション
- **ノード追加**: Tab（子ノード＝既定は右）/ Enter（兄弟ノード）。**矢印キーを押しながら Tab** を押すと、その向き（→右 / ←左 / ↑上 / ↓下）に子を生やす。右クリックメニューから方向指定（右/左/上/下）でも追加可。
...
- **右クリックメニュー**:
  - ノード上: **2段構成**。トップ階層は常時1列（名前を変更／ノート追加・編集・削除／削除＋カテゴリ）で、各カテゴリ行に**カーソルを乗せると右側にフライアウト**で詳細が開く（クリック不要のホバー展開）。
...
interface MindMap {
  ...
  organicStyle?: boolean;          // 全体を「枠なし手描き風」で表示（ページ設定）
...
interface MindMapNode {
  ...
  edgeLabelFontSize?: number;      // 線ラベルの文字サイズ（省略時 12）
}
```
...
- `Connection { id, from, to, color?, label?, labelFontSize? }` … ノード同士の関連線（from/to はノード id、label は線中央のラベル文字、labelFontSize は大きさ）
